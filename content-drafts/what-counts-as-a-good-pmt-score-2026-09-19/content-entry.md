Ready to splice into the `articles` array in `src/data/content.ts` (placed after `nser-pmt-score-check-guide`, before the closing `];`). Types match the current `Article`/`ContentSection` definitions (including `subsections` and `table`).
